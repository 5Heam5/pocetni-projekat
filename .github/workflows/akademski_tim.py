...
class Pipe:
    ...
    # OBAVEZNA METODA - OpenWebUI traži pipelines() a ne pipes()
    def pipelines(self):
        """Vraća listu dostupnih pipeline-ova."""
        return [
            {
                "id": "triple_threat_v2", 
    ...
    # OBAVEZNA METODA - za dobavljanje trenutnih podešavanja
    async def get_valves(self):
        return self.valves.dict()

    # OBAVEZNA METODA - za ažuriranje podešavanja
    async def update_valves(self, **valves):
        try:
            ...
            # Ažuriraj klijenta sa novim URL-om
            self.client = AsyncOpenAI(base_url=self.valves.OLLAMA_URL, api_key="ollama")
            return await self.get_valves()
        except Exception as e:
            raise Exception(f"Greška pri ažuriranju podešavanja: {str(e)}")

    # Opcionalne lifecycle metode
    async def on_startup(self):
        """Poziva se pri pokretanju pipeline-a."""
        print("🚀 Akademski Tim 2025 pipeline je pokrenut")

    async def on_shutdown(self):
        """Poziva se pri gašenju pipeline-a."""
        print("🛑 Akademski Tim 2025 pipeline je zaustavljen")
    ...
    async def _stream(self, model: str, prompt: Union[str, list[dict]], emitter):
        ...
        try:
            ...
            error_message = f"\n\n[GREŠKA] Komunikacija sa modelom '{model}' neuspešna: {str(e)}\n"
    ...
    async def pipe(self, body: dict, __event_emitter__):
        """Glavna metoda pipeline-a koja se izvršava na korisnički zahtev."""
        self._cleanup_memory()

        # ─────────────── OSNOVNA VALIDACIJA ───────────────
        if not body.get("messages"):
            await __event_emitter__({"type": "content", "content": "GREŠKA: Nema poruka u zahtevu."})
            return

        user_msg = body["messages"][-1]["content"] if body.get("messages") else ""
        if not user_msg or not str(user_msg).strip():
            await __event_emitter__({"type": "content", "content": "GREŠKA: Prazan tekst. Molimo unesite sadržajan upit."})
            return
        ...
        await __event_emitter__({"type": "content", "content": "🚀 Pokrećem Akademski Tim 2025 – Triple Threat Edition v2...\n\n"})

        # ────────────────────── 1. AKADEMIK ──────────────────────
        await __event_emitter__({"type": "content", "content": "1️⃣ **AKADEMIK** – Analiza teme i izrada outline-a\n"})
        
        akademik_content = [{"type": "text", "text": f"Napravi detaljan akademski plan i outline (strukturu) za temu:\n\n{user_msg}"}]
        
        # Dodaj slike ako postoje
        if images:
            for img in images:
                akademik_content.append({"type": "image_url", "image_url": {"url": img}})

        akademik_msg = [{"role": "user", "content": akademik_content}]

        outline = await self._stream(self.valves.MODEL_AKADEMIK, akademik_msg, __event_emitter__)
        
        if not outline.strip():
            await __event_emitter__({"type": "content", "content": "\n\n❌ GREŠKA: Akademik nije generisao outline. Proces se prekida."})
            return
        ...
        # ────────────────────── 2. PISAC ──────────────────────
        await __event_emitter__({"type": "content", "content": "2️⃣ **PISAC** – Pisanje prvog drafta\n"})
        
        pisac_prompt = f"""Piši kompletan akademski rad na srpskom jeziku.
Striktno prati sledeći outline:
{outline}

Zahtevi:
- Originalan, naučan stil pisanja
- Koristi APA 7 stil citiranja
- Ciljna dužina: 2500–4000 reči
- Fokusiraj se na kvalitetan sadržaj
- Koristi akademski jezik i terminologiju"""

        draft = await self._stream(self.valves.MODEL_PISAC, pisac_prompt, __event_emitter__)
        
        if not draft.strip():
            await __event_emitter__({"type": "content", "content": "\n\n❌ GREŠKA: Pisac nije generisao draft. Proces se prekida."})
            return
        ...
        # ────────────────────── 3. LEKTOR ──────────────────────
        await __event_emitter__({"type": "content", "content": "3️⃣ **LEKTOR** – Finalna obrada, lekturanje i formatiranje\n"})
        
        draft_tokens = self._estimate_tokens(draft)
        await __event_emitter__({"type": "content", "content": f"[INFO] Draft ima ~{draft_tokens} tokena. Proveravam da li je potrebno deljenje...\n"})
        
        if draft_tokens > self.valves.MAX_TOKENS_PER_CHUNK:
            await __event_emitter__({"type": "content", "content": f"✂️ Tekst je predugačak, delimo ga na manje delove za lekturanje...\n"})
            final_parts = []
            chunks = self._split_text_into_chunks(draft, self.valves.MAX_TOKENS_PER_CHUNK)

            for i, chunk in enumerate(chunks):
                is_last = i == len(chunks) - 1
                await __event_emitter__({"type": "content", "content": f"\n🔨 Obradjujem deo {i+1}/{len(chunks)}...\n"})
...
3. DODAJ na sam početak celog rada sažetak (Abstract) i ključne reči
4. Na sam kraj celog rada dodaj formatiranu Literaturu prema APA 7 standardu
5. Vrati ISKLJUČIVO obrađen deo teksta
...
1. Popravi gramatiku, stil, ponavljanja
2. Osiguraj 100% srpski jezik
3. NE DODAVAJ sažetak, ključne reči ili literaturu
4. Vrati ISKLJUČIVO obrađen deo teksta

Deo za lekturanje:
{chunk}"""

                final_part = await self._stream(self.valves.MODEL_LEKTOR, lektor_prompt, __event_emitter__)
                if final_part:
...
4. Dodati na početak sažetak (Abstract) i ključne reči
...
{draft}"""
            ...
            final = await self._stream(self.valves.MODEL_LEKTOR, lektor_prompt, __event_emitter__)
