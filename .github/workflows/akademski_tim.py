...
class Pipeline:  # <-- OVO JE ISPRAVNO!
    """
    ...
    # KLJUČNA ISPRAVKA: Ova metoda se MORA zvati pipelines() a ne pipes()
    def pipelines(self):  # <-- PROMENI OVO IME!
    ...
    async def pipe(self, body: dict, __event_emitter__):
        # OVA METODA OSTAJE ISTA - OVO JE GLAVNA LOGIKA
        self._cleanup_memory()

        if not body.get("messages"):
            await __event_emitter__({"type": "content", "content": "GRESKA: Nema poruka u zahtevu."})
            return

        user_msg = body["messages"][-1]["content"] if body.get("messages") else ""
        if not user_msg or not str(user_msg).strip():
            await __event_emitter__({"type": "content", "content": "GRESKA: Prazan tekst. Molimo unesite sadržajan upit."})
            return
        ...
        await __event_emitter__({"type": "content", "content": "🚀 Pokrecem Akademski Tim 2025 – Triple Threat Edition v2...\n\n"})
...
- Fokusiraj se na sadržaj, ne na formatiranje."""
        ...
        if draft_tokens > self.valves.MAX_TOKENS_PER_CHUNK:
            await __event_emitter__({"type": "content", "content": f"✂️ Tekst je predugacak, delimo ga na manje delove za lekturanje...\n"})
            final_parts = []
...
3. DODAJ na sam pocetak celog rada sažetak (Abstract) i ključne reci.
...
1. Popravi gramatiku, stil, ponavljanja.
2. Osiguraj 100% srpski jezik.
3. NE DODAVAJ sažetak, ključne reci ili literaturu.
